...
function affinityProviderFor(providerName: string): AliasProvider | undefined {
  if (providerName === "codex" || providerName === "kimi" || providerName === "zai" || providerName === "anthropic") return providerName as AliasProvider;
  return undefined;
}
...
async function route(req: Request, url: URL, reqId: string): Promise<Response> {
  ...
  if (req.method === "GET" && url.pathname === "/v1/models") {
    const models = allSupportedModels().map(m => ({
      type: "model",
      id: m.model,
      display_name: `${m.provider}: ${m.model}`,
      created_at: new Date().toISOString()
    }));
    return new Response(JSON.stringify({ data: models, has_more: false }), {
      headers: { "content-type": "application/json" }
    });
  }
